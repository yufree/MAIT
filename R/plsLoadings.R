...
plsLoadings<-function(MAIT.object){
  if(length(MAIT.object@FeatureData@plsModel)==1){
  out <- (model(MAIT.object,type="PLS")$finalModel$loadings)
}else{
stop("No  available PLS model. Run function plotPLS.")
}
    return(out)
}
