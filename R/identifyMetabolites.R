...
identifyMetabolites <- function(MAIT.object = NULL,
        ...
        if (length(featureSigID(MAIT.object)) == 0) {
                stop(
                        "No significant features found in the MAIT object. Make sure that functions peakAnnotation and spectralSignFeatures were launched"
                )
        }
        
        if (length(MAIT.object@FeatureData@masses) == 0 &
            length(rawData(MAIT.object)) == 0) {
        ...
        if (is.null(database)) {
                ...
                dataBase <-
                        read.csv(paste(database, ".csv", sep = ""),
                                 sep = ",",
                                 header = TRUE)
        ...
        MAIT.object@RawData@parameters@identifyMetabolites <-
                parameters
        ...
        aux <-
                signPeaklist[c(
                        -grep("[M+1]", signPeaklist$isotope, fixed = TRUE),
                        -grep("[M+2]", signPeaklist$isotope, fixed = TRUE)
                ), ]
        ...
        if (length(rawData(MAIT.object)) == 0) {
                peaksIsTP <-
                        matrix(nrow = 1,
                               ncol = dim(aux)[2] + 2 * length(classes(MAIT.object)))
                colnames(peaksIsTP) <-
                        c(
                                colnames(aux)[3:(dim(aux)[2] - 1)],
                                "p.adj",
                                "p",
                                colnames(sigPeaksTable)[dim(sigPeaksTable)[2] - 1 - length(classes(MAIT.object)) *
                                                                2],
                                colnames(sigPeaksTable)[(dim(sigPeaksTable)[2] - 2 * length(classes(MAIT.object)) +
                                                                 1):dim(sigPeaksTable)[2]]
                        )
                
        } else{
                peaksIsTP <-
                        matrix(nrow = 1,
                               ncol = dim(aux)[2] - 7 + 2 * length(classes(MAIT.object)))
                colnames(peaksIsTP) <-
                        c(
                                colnames(aux)[8:(dim(aux)[2] - 3)],
                                "p.adj",
                                "p",
                                colnames(sigPeaksTable)[dim(sigPeaksTable)[2] - length(classes(MAIT.object)) *
                                                                2],
                                colnames(sigPeaksTable)[(dim(sigPeaksTable)[2] - 2 * length(classes(MAIT.object)) +
                                                                 1):dim(sigPeaksTable)[2]]
                        )
        ...
        for (i in (1:dim(sigPeaksTable)[1])) {
                ...
                if (SearchPeak$unk == 0) {
                        if (length(SearchPeak$SearchCand) == 5) {
                                ...
                                SearchPeak$SearchCand <- matrix(SearchPeak$SearchCand,
                                                                nrow = 1)
                        } else{
                                ...
                                if (length(rawData(MAIT.object)) == 0) {
                                        Search <-
                                                rbind(
                                                        ...
                                                        cbind(
                                                                round(
                                                                        sigPeaksTable$mz[index],
                                                                        5
                                                                ),
                                                                as.numeric(
                                                                        SearchPeak$SearchCand[k, 4]
                                                                ),
                                                                round(
                                                                        sigPeaksTable$rt[index],
                                                                        2
                                                                ),
                                                                sigPeaksTable$adduct[index],
                                                                as.character(
                                                                        SearchPeak$SearchCand[k, 2]
                                                                ),
                                                                spectra,
                                                                as.character(
                                                                        SearchPeak$SearchCand[k, 5]
                                                                ),
                                                                as.character(
                                                                        SearchPeak$SearchCand[k, 1]
                                                                )
                                                        )
                                                )
                                        ...
                                        if (length(classes(MAIT.object)) >= 2) {
                                                ...
                                                added <-
                                                        cbind(
                                                                ...
                                                                sigPeaksTable$P.adjust[index],
                                                                sigPeaksTable$p[index],
                                                                sigPeaksTable[index, grep(
                                                                        "Fisher.",
                                                                        colnames(
                                                                                sigPeaksTable
                                                                        )
                                                ...
                                                added <-
                                                        cbind(
                                                                ...
                                                                sigPeaksTable$P.adjust[index],
                                                                sigPeaksTable$p[index],
                                                                sigPeaksTable$Fisher.Test[index],
                                                                sigPeaksTable[index, (
                                                                        dim(
                                                                                sigPeaksTable
                                                                        )[2] - 2 * length(
                                        ...
                                        added <-
                                                cbind(
                                                        sigPeaksTable[index, 8:(
                                                                dim(sigPeaksTable)[2] - 6 - length(
                                                                        classes(MAIT.object)
                                                                ) * 2
                                                        )],
                                                        sigPeaksTable$P.adjust[index],
                                                        sigPeaksTable$p[index],
                                                        sigPeaksTable[index, dim(sigPeaksTable)[2] - length(classes(MAIT.object)) *
                                                                              2],
                                                        sigPeaksTable[index, (
                                                                dim(sigPeaksTable)[2] - 2 * length(
                                                                        classes(MAIT.object)
                                                                ) + 1
                                                        ):dim(sigPeaksTable)[2]]
                                                )
                        ...
                        if (length(rawData(MAIT.object)) == 0) {
                                Search <-
                                        rbind(
                                                Search,
                                                cbind(
                                                        round(sigPeaksTable$mz[index], 5),
                                                        "Unknown",
                                                        round(sigPeaksTable$rt[index], 2),
                                                        sigPeaksTable$adduct[index],
                                                        "Unknown",
                                                        spectra,
                                                        as.character(SearchPeak$SearchCand[5]),
                                                        as.character(SearchPeak$SearchCand[1])
                                                )
                                        )
                                ...
                                added <-
                                        cbind(
                                                sigPeaksTable[index, 3:(dim(sigPeaksTable)[2] - 5 - length(classes(
                                                        MAIT.object
                                                )) * 2)],
                                                sigPeaksTable$P.adjust[index],
                                                sigPeaksTable$p[index],
                                                sigPeaksTable[index, grep("Fisher.",
                                                                          colnames(sigPeaksTable))],
                                                sigPeaksTable[index, (dim(sigPeaksTable)[2] - 2 * length(classes(
                                                        MAIT.object
                                                )) + 1):dim(sigPeaksTable)[2]]
                                        )
                                colnames(added) <- colnames(peaksIsTP)
        ...
        metaboliteTable <-
                merge(Search.df, peaksIsTP.df, by = "row.names")[, -1]
        metaboliteTable[, 1] <-
                round(as.numeric(as.character(metaboliteTable[, 1])), 4)
        #aux <- metaboliteTable
        ...
        if (length(rawData(MAIT.object)) == 0) {
                ind1 <- 9:(dim(metaboliteTable)[2] - 2 * length(classes(MAIT.object)) -
                                   3)
                ind2 <-
                        (dim(metaboliteTable)[2] - 2 * length(classes(MAIT.object)) - 2):dim(metaboliteTable)[2]
                tem <- metaboliteTable[, ind2]
                metaboliteTable[, (dim(metaboliteTable)[2] + 1 - length(ind1)):dim(metaboliteTable)[2]] <-
                        metaboliteTable[, ind1]
                colnames(metaboliteTable)[(dim(metaboliteTable)[2] + 1 - length(ind1)):dim(metaboliteTable)[2]] <-
                        colnames(metaboliteTable)[ind1]
                metaboliteTable[, (dim(metaboliteTable)[2] + 1 - length(ind1) - length(ind2)):(dim(metaboliteTable)[2] -
                                                                                                       length(ind1))] <- tem
                colnames(metaboliteTable)[(dim(metaboliteTable)[2] + 1 - length(ind1) -
                                                   length(ind2)):(dim(metaboliteTable)[2] - length(ind1))] <-
                        colnames(tem)
                
        } else{
                ind1 <-
                        10:(dim(metaboliteTable)[2] - 2 * length(classes(MAIT.object)) - 3)
                ind2 <-
                        (dim(metaboliteTable)[2] - 2 * length(classes(MAIT.object)) - 2):dim(metaboliteTable)[2]
                tem <- metaboliteTable[, ind2]
                metaboliteTable[, (dim(metaboliteTable)[2] + 1 - length(ind1)):dim(metaboliteTable)[2]] <-
        ...
        if (printCSVfile == TRUE) {
                if (!file.exists(paste(resultsPath, "Tables", sep = "/"))) {
                        ...
                        warning(
                                paste(
                                        "Folder",
                                        paste(resultsPath, "Tables", sep = "/"),
                                        "already exists. Possible file overwritting.",
                                        sep = " "
                                )
                        )
                        write.table(
