...
Biotransformations <- function(MAIT.object = NULL,
        ...
        if (length(featureSigID(MAIT.object)) == 0) {
                stop(
                        "No significant features found in the MAIT object. Make sure that functions peakAnnotation and peakAggregation were launched"
                )
        }
        ...
        if (is.null(bioTable)) {
                ...
                biotransformationsTable <-
                        get("biotransformationsTable", envir = peakBioEnv)
                ...
                biotransformationsTable <-
                        as.data.frame(read.csv(
                                paste(bioTable, ".csv", sep = ""),
                                sep = ",",
                                header = TRUE,
                                row.names = NULL
                        ))
                cat("Selecting user-defined Biotransformations Table",
                    fill = TRUE)
        ...
        if (adductAnnotation) {
                if (is.null(adductTable)) {
                        ...
                        if (adductTable == "negAdducts") {
                                cat(
                                        "adductTable has been set to negAdducts. The default MAIT adducts table for negative polarization is selected...",
                                        fill = TRUE
                                )
                                
                                peakAnnEnv <- new.env()
                                data(MAITtables, envir = peakAnnEnv)
                                adducts <- get(x = "negAdducts",
                                               envir = peakAnnEnv)
                                adducts$massdiff <- adducts$massdiff - adducts$charge * H.mass
        ...
        MAIT.object@RawData@parameters@biotransformations <-
                parameters
        ...
        index <- matrix(nrow = 1)
        colnames(BiotransformationsTable) <-
                c(
                        "Mass1",
                        "Mass2",
                        "Adduct",
        ...
        allSpectra <-
                sapply(signIndex, FUN = retrieveSpectrum, sigPeaksTable = sigPeaksTable)
        ...
        if (adductAnnotation) {
                ...
                addRange <-
                        cbind(
                                adducts$massdiff - peakPrecision,
                                adducts$massdiff + peakPrecision,
                                adducts$nmol
                        )
                ...
                for (k in 1:length(signIndex)) {
                        ...
                        if (length(spectrum) > 1) {
                                names(spectrum) <- match(round(spectrum, 4),
                                                         round(sigPeaksTable$mz, 4))
        ...
        if (class(BiotransformationsTable) != "list") {
                MAIT.object@FeatureInfo@biotransformations <-
                        BiotransformationsTable
        } else{
                aux <- names(BiotransformationsTable)
                BiotransformationsTable <-
                        matrix(BiotransformationsTable,
                               nrow = 1,
                               ncol = 7)
...
spectrumAnnotation <-
        function(testValues,
                ...
                for (i in c(1:length(testValues))) {
                        ...
                        if (!is.null(diffSpectrum)) {
                                testValues <- diffSpectrum
                        }
...
inBetween <- function(testValue, biotRange) {
        out <-
                rownames(biotRange)[which(testValue <= biotRange[, 2] &
                                                  testValue >= biotRange[, 1])]
...
annotateBiotransf <-
        function(biotransf,
                ...
                if (length(unlist(biotransf)) > 1) {
                        biotransfMass <-
                                biotransformationsTable$MASS[match(table = biotransformationsTable$NAME,
                                                                   x = unlist(biotransf)[1])]
                        biotransfName <- character(length = 1)
                        for (i in c(1:length(unlist(biotransf)))) {
                                biotransfName <- paste(biotransfName,
                                                       unlist(biotransf)[i],
                                                       ",")
                        }
                } else{
                        biotransfMass <-
                                biotransformationsTable$MASS[match(table = biotransformationsTable$NAME, x =
                                                                           biotransf)]
                }
                auxIndex2 <-
                        as.numeric(names(spectrum)[which(names(spectrum) %in% names(biotransf))])
                auxIndex1 <-
                        as.numeric(names(spectrum)[which(names(spectrum) %in% names(biotransf)) -
                                                           diffIndex])
                ...
                rtPeak2 <-
                        sigPeaksTable$rt[match(table = sigPeaksTable$mz, x = sigPeaksTable$mz[auxIndex2])]
                signPeakIndex2 <-
                        match(table = sigPeaksTable$mz, x = sigPeaksTable$mz[auxIndex2])
                rtPeak1 <-
                        sigPeaksTable$rt[match(table = sigPeaksTable$mz, x = sigPeaksTable$mz[auxIndex1])]
                signPeakIndex1 <-
                        match(table = sigPeaksTable$mz, x = sigPeaksTable$mz[auxIndex1])
                if (biotransfMass > 0) {
                        if (length(unlist(biotransf)) > 1) {
                                annotation <-
                                        paste(
                                                "[M",
                                                round(as.numeric(massPeak1), 3),
                                                "_rt",
                                                round(as.numeric(rtPeak1), 2),
                                                "] ",
                                                biotransfName,
                                                sep = ""
                                        )
...
retrieveSpectrum <-
        function(spectrumNumber, sigPeaksTable) {
                return(sigPeaksTable[which(as.numeric(sigPeaksTable$pcgroup) == spectrumNumber), 1])
        }
                                        ...
                                        if (!is.null((
                                                ...
                                                for (k in c(1:length(an))) {
                                                        if (length(unlist(an[k])) >= 1) {
                                                                ...
                                                                mass2 <- sigPeaksTable$mz[as.numeric(names(
                                                                        spectrum
                                                                ))[j]]
                                                                rtPeak1 <- sigPeaksTable$rt[as.numeric(names(an))[k]]
                                                                rtPeak2 <- sigPeaksTable$rt[as.numeric(names(
                                                                        spectrum
                                                                ))[j]]
                                                                signPeakIndex2 <- match(
                                                                        table = sigPeaksTable$mz,
                                                                        x = mass1
                                                                )
                                                                signPeakIndex1 <- match(
                                                                        table = sigPeaksTable$mz,
                                                                        x = mass2
                                                                )
